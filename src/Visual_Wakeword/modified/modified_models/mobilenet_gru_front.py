...
class ModifiedRNNPool(RNNPool):
    ...
    def _build(self):
        self.cell_rnn = GRU(self.inputDims, self.nHiddenDims, gate_nonlinearity="sigmoid",
                            update_nonlinearity="tanh", wSparsity=self.w1Sparsity, uSparsity=self.u1Sparsity,
                            batch_first=False, bidirectional=False)

        self.cell_bidirrnn = GRU(self.nHiddenDims, self.nHiddenDimsBiDir, gate_nonlinearity="sigmoid",
...
class MobileNetV2(nn.Module):
    def __init__(self, 
        ...
        self.rnn_model = ModifiedRNNPool(6, 6, 8, 8, input_channel)#num_init_features)
